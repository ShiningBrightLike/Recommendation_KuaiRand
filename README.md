...
python baselines.py --models "mmoe+mlp,mmoe+dcn,mmoe+senet"   # 同一结构下对比三种编码器
```
...
## 📊 对照模型 v2（2026-09-15，新默认模型 × 3 seed）

ADR-0005 之后默认模型是「特征编码器 `mlp` → MMoE」，下表在同一协议下重刷：seeds 2025 / 2026 / 2027，`val_auc_mean` 早停，测试集每个配置只评估一次，数字为 3 个 seed 的 mean±样本标准差（ddof=1）。
...
观察（结论以门控任务口径为准——信号任务是点击/点赞；关注、评论标签稀疏、波动大，只作参考）：

- **`mmoe+senet` 在两个口径上都最好**（四任务均值 0.7170，门控均值 0.7695），点击、点赞、评论三项均为最优。但它相对默认 `mmoe+mlp` 的领先只有 +0.0038（四任务均值）与 +0.0046（门控均值），而 seed 间标准差是 ±0.0064～±0.0077——**优势尚未超出种子噪声**，需要更多 seed 才能下结论。
- **`mmoe+dcn` 是 MMoE 三种编码器里最差的**（0.7076±0.0142，波动最大），而它的编码器参数是 `mlp` 的 5 倍（108,900 vs 21,696）、整模型参数 338,632 vs 67,556。低秩交叉在这份特征集上**没有换来收益，只放大了方差**（主要是关注任务 0.6707±0.0396）。
- 另外四个「默认编码器」模型（shared_bottom / single_task / mmoe+mlp / mmoe+dcn）四任务均值落在 0.708–0.714、门控均值 0.7649–0.7670，差异都在种子噪声量级内——**多任务结构之间没有明显赢家**，这比 v1 单 seed 得出的「单任务领先」更弱化。
- logistic 依旧明显落后（0.6936），证明 Embedding + 非线性表征确有增益。

完整结果见 [baselines_v2.md](docs/assets/baselines_v2.md) 与 [baselines_v2.json](docs/assets/baselines_v2.json)（含每个 seed 的逐任务 AUC、best epoch 与耗时）。与 v1 表**不可直接比较**：v1 用的是旧默认模型（没有编码器）且只有单个 seed。

---

## ⚖️ 对照模型与统计特征泄漏审计（2026-09-13，旧默认模型，已被上面的 v2 取代）
