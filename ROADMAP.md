...
| #7 | 发布 baseline v2 对照表（多 seed mean±std） | 已关闭：2026-09-16 验证集合规重跑并发布 |
| #8 | 用新默认模型重跑置换重要度并重判特征门控 | 已关闭 |

结论见 §9 的 2026-09-14/15/16 记录，结果产物见 `docs/assets/baselines_v2.*` 与 `docs/assets/feature_importance_v2*`。
...
- **baseline v2 历史结果（2026-09-15，3 seed × 6 配置；2026-09-16 判定协议失效）**：

  > 复审发现该批次用最终确认集比较了多个候选配置，违反“验证集选型、配置锁定后才使用最终确认集”的纪律。以下数字仅作历史审计，不支持模型优劣结论；`baselines.py` 已改为默认验证集、`--final-test` 只允许一个锁定配置，并为每个配置/seed 保存独立模型和 metadata。合规替代结果已于 2026-09-16 完成，见下文。
  ...
  - 数值层面 `senet` 较高、`dcn` 较低，但不得据此选型；需在验证集重跑后重新判断。
  - v1/v2 数字不可直接比较；本批次也不能与未来合规验证集报告直接比较。
  - 原发布资产已由 2026-09-16 的合规验证集批次替换；本失效批次只在实验台账 E6 与 Git 历史中保留。
...
- **baseline v2 合规重跑完成（T6 / issue #7）**：run `baseline-v2-val-3seed-20260916`，6 配置 × seeds 2025/2026/2027，全量 train/val = 950,310 / 190,802，30 epochs 上限、`val_auc_mean` 早停。候选比较只读取验证集，最终确认集未加载。
